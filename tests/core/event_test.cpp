...
{
    Delegate<int(int)> doubler{[](int x) {
        return x * 2;
    }};
    CHECK(doubler(7) == 14);
    CHECK(doubler(-3) == -6);
...
TEST_CASE("Signal: connected slots are invoked on emit")
{
    Signal<int> s;
    int         sum = 0;
    s.connect(Delegate<void(int)>{[&sum](int x) {
        sum += x;
    }});
    s.connect(Delegate<void(int)>{[&sum](int x) {
        sum += 10 * x;
...
TEST_CASE("Signal: disconnect removes a single slot by id")
{
    Signal<> s;
    int      a = 0, b = 0;
    SlotId   id_a = s.connect(Delegate<void()>{[&]() {
        ++a;
    }});
    SlotId   id_b = s.connect(Delegate<void()>{[&]() {
        ++b;
    }});
    (void)id_b;
...
TEST_CASE("EventBus: routes events by type, ignoring others")
{
    ...
    bus.subscribe<DamageEvent>(Delegate<void(const DamageEvent&)>{[&](const DamageEvent& e) {
        total_damage += e.amount;
    }});
    bus.subscribe<HealEvent>(Delegate<void(const HealEvent&)>{[&](const HealEvent& e) {
        total_heal += e.amount;
    }});
...
TEST_CASE("EventBus: unsubscribe stops further delivery")
{
    EventBus bus;
    int      hits = 0;
    SlotId   id   = bus.subscribe<DamageEvent>(Delegate<void(const DamageEvent&)>{[&](const DamageEvent&) {
        ++hits;
    }});
