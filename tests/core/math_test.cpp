...
TEST_CASE("Quat: 90-degree rotation around Z maps +X -> +Y") {
    Quat q = Quat::from_axis_angle(Vector3{0.0f, 0.0f, 1.0f},
                                   static_cast<f32>(PI * 0.5));
...
TEST_CASE("Transform3D: inverse round-trip") {
    Transform3D t{Vector3{5.0f, -3.0f, 7.0f},
                  Quat::from_axis_angle(Vector3{0.0f, 1.0f, 0.0f}, 0.4f),
