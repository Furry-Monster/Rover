...
namespace rover
{
    ...
    bool Window::init(const WindowDesc& desc)
    {
        ...
        window_ = SDL_CreateWindow(desc.title, static_cast<int>(desc.width), static_cast<int>(desc.height), flags);
    ...
    void Window::get_vulkan_required_extensions(std::vector<const char*>& out)
    {
        ...
        if (exts == nullptr)
        {
            ROVER_LOG_ERROR("SDL_Vulkan_GetInstanceExtensions failed: {}", SDL_GetError());
            return;
        }
