...
void TimeSource::tick() {
    ...
    const u64 ticks = (current_counter_ > last_counter_)
                          ? (current_counter_ - last_counter_)
                          : 0;
    delta_seconds_ = (frequency_ > 0)
                         ? static_cast<f64>(ticks) / static_cast<f64>(frequency_)
                         : 0.0;
    ++frame_count_;
}
