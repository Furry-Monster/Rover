...
i32 score_device(VkPhysicalDevice physical, VkSurfaceKHR surface) {
    if (!device_supports_extensions(physical)) return -1;
    if (!surface_has_formats(physical, surface)) return -1;
    if (!find_queue_families(physical, surface).is_complete()) return -1;
...
bool VkDeviceWrapper::init(VkInstance instance, VkSurfaceKHR surface) {
    ...
    ROVER_LOG_INFO("Vulkan logical device created (graphics_q={}, present_q={})",
                   queue_families_.graphics, queue_families_.present);
    return true;
}
