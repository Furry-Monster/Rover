...
namespace rover
{
    ...
    PipelineHandle GraphicsDeviceVulkan::create_graphics_pipeline(const GraphicsPipelineDesc& desc)
    {
        ...
        // Pipeline layout: prefer the explicitly-passed handle; otherwise create
        // an anonymous empty layout owned by this pipeline (Phase 1 demo path).
        VkPipelineResource res{};
        VkPipelineLayout   vk_layout = VK_NULL_HANDLE;
        if (desc.pipeline_layout != INVALID_HANDLE)
        {
            auto* pl = pipeline_layouts_.get(desc.pipeline_layout);
            if (pl == nullptr || pl->layout == VK_NULL_HANDLE)
            {
                ROVER_LOG_ERROR("create_graphics_pipeline: invalid pipeline_layout");
                return INVALID_HANDLE;
            }
            vk_layout       = pl->layout;
            res.layout      = desc.pipeline_layout;
            res.owns_layout = false;
        }
        else
        {
            VkPipelineLayoutCreateInfo layout_info{};
            layout_info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
            if (vkCreatePipelineLayout(device_.logical(), &layout_info, nullptr, &vk_layout) != VK_SUCCESS)
            {
                ROVER_LOG_ERROR("vkCreatePipelineLayout failed");
                return INVALID_HANDLE;
            }
            // Wrap the anonymous layout in the pipeline_layouts_ pool so the
            // command-side path can resolve it from `res.layout` uniformly.
            VkPipelineLayoutResource plres{};
            plres.layout    = vk_layout;
            res.layout      = pipeline_layouts_.add(plres);
            res.owns_layout = true;
        }
        ...
        pipeline_info.layout              = vk_layout;
        pipeline_info.renderPass          = rp->pass;
        pipeline_info.subpass             = 0;

        if (vkCreateGraphicsPipelines(device_.logical(), VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &res.pipeline) !=
            VK_SUCCESS)
        {
            ROVER_LOG_ERROR("vkCreateGraphicsPipelines failed");
            if (res.owns_layout)
            {
                destroy_pipeline_layout(res.layout);
            }
            return INVALID_HANDLE;
        }
    ...
    {
        VkPipelineResource res{};
        if (!pipelines_.remove(handle, &res))
        {
            return;
        }
        ...
        if (res.owns_layout && res.layout != INVALID_HANDLE)
        {
            destroy_pipeline_layout(res.layout);
        }
    }
