...
bool GraphicsDeviceVulkan::init(WindowSystem& window) {
    ...
    if (!swapchain_.init(instance_.handle(), device_.physical(), device_.logical(),
                         surface_, device_.queue_families(),
    ...
    for (u32 i = 0; i < kMaxFramesInFlight; ++i) {
        VK_CHECK_RETURN(vkCreateSemaphore(device_.logical(), &sem_info, nullptr,
                                          &frame_sync_[i].image_available), false);
        VK_CHECK_RETURN(vkCreateFence(device_.logical(), &fence_info, nullptr,
                                      &frame_sync_[i].in_flight), false);
    }
...
void GraphicsDeviceVulkan::recreate_swapchain() {
    ...
    if (!swapchain_.recreate(device_.physical(), device_.logical(), surface_,
