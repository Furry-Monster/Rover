...
namespace rover
{
    ...
    VkBufferUsageFlags to_vk_buffer_usage(BufferUsage u)
    {
        ...
        if (bits & static_cast<U>(BufferUsage::Vertex))
        {
            out |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        }
        if (bits & static_cast<U>(BufferUsage::Index))
        {
            out |= VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
        }
        if (bits & static_cast<U>(BufferUsage::Uniform))
        {
            out |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
        }
        if (bits & static_cast<U>(BufferUsage::Storage))
        {
            out |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
        }
        if (bits & static_cast<U>(BufferUsage::Indirect))
        {
    ...
    VkImageUsageFlags to_vk_image_usage(TextureUsage u)
    {
        ...
        if (bits & static_cast<U>(TextureUsage::Sampled))
        {
            out |= VK_IMAGE_USAGE_SAMPLED_BIT;
        }
        if (bits & static_cast<U>(TextureUsage::Storage))
        {
            out |= VK_IMAGE_USAGE_STORAGE_BIT;
        }
        if (bits & static_cast<U>(TextureUsage::ColorAttachment))
        {
            out |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
        }
        if (bits & static_cast<U>(TextureUsage::DepthStencilAttachment))
        {
            out |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        }
        if (bits & static_cast<U>(TextureUsage::TransferSrc))
        {
            out |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        }
        if (bits & static_cast<U>(TextureUsage::TransferDst))
        {
    ...
    VkShaderStageFlags to_vk_shader_stage_flags(ShaderStage s)
    {
        ...
        if (bits & static_cast<U>(ShaderStage::Vertex))
        {
            out |= VK_SHADER_STAGE_VERTEX_BIT;
        }
        if (bits & static_cast<U>(ShaderStage::Fragment))
        {
            out |= VK_SHADER_STAGE_FRAGMENT_BIT;
        }
        if (bits & static_cast<U>(ShaderStage::Compute))
        {
            out |= VK_SHADER_STAGE_COMPUTE_BIT;
        }
        if (bits & static_cast<U>(ShaderStage::Geometry))
        {
            out |= VK_SHADER_STAGE_GEOMETRY_BIT;
        }
        if (bits & static_cast<U>(ShaderStage::TessControl))
        {
            out |= VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
        }
        if (bits & static_cast<U>(ShaderStage::TessEval))
        {
