...
void GraphicsDeviceVulkan::end_command_list(CommandListHandle cmd) {
    ...
    if (res == nullptr || !res->recording) return;
...
void GraphicsDeviceVulkan::cmd_begin_render_pass(CommandListHandle cmd, FramebufferHandle fb,
                                                  const ClearValue* clear_values, u32 clear_count) {
    auto* cl = command_lists_.get(cmd);
    auto* fb_res = framebuffers_.get(fb);
    if (cl == nullptr || fb_res == nullptr) return;
    ...
    for (u32 i = 0; i < clear_count; ++i) {
        ...
        const bool is_depth = rp->has_depth && i + 1 == clear_count
                              && i >= rp->color_formats.size();
        if (is_depth) {
...
void GraphicsDeviceVulkan::cmd_bind_pipeline(CommandListHandle cmd, PipelineHandle pipeline) {
    ...
    if (cl == nullptr || p == nullptr) return;
...
void GraphicsDeviceVulkan::cmd_bind_vertex_buffer(CommandListHandle cmd, BufferHandle buffer,
                                                   u32 binding, usize offset) {
    auto* cl  = command_lists_.get(cmd);
    auto* buf = buffers_.get(buffer);
    if (cl == nullptr || buf == nullptr) return;
    VkDeviceSize off = offset;
    vkCmdBindVertexBuffers(cl->buffer, binding, 1, &buf->buffer, &off);
}

void GraphicsDeviceVulkan::cmd_bind_index_buffer(CommandListHandle cmd, BufferHandle buffer,
                                                  IndexType type, usize offset) {
    auto* cl  = command_lists_.get(cmd);
    auto* buf = buffers_.get(buffer);
    if (cl == nullptr || buf == nullptr) return;
...
void GraphicsDeviceVulkan::cmd_draw(CommandListHandle cmd, u32 vertex_count,
                                     u32 instance_count, u32 first_vertex, u32 first_instance) {
    auto* cl = command_lists_.get(cmd);
    if (cl == nullptr) return;
    vkCmdDraw(cl->buffer, vertex_count, instance_count, first_vertex, first_instance);
}

void GraphicsDeviceVulkan::cmd_draw_indexed(CommandListHandle cmd, u32 index_count,
                                             u32 instance_count, u32 first_index,
                                             i32 vertex_offset, u32 first_instance) {
    auto* cl = command_lists_.get(cmd);
    if (cl == nullptr) return;
    vkCmdDrawIndexed(cl->buffer, index_count, instance_count, first_index,
