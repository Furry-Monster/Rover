...
namespace rover
{
    ...
    bool VkSwapchainWrapper::init(VkInstance /*instance*/,
    ...
    bool VkSwapchainWrapper::recreate(VkPhysicalDevice          physical,
                                      VkDevice                  device,
                                      VkSurfaceKHR              surface,
                                      const QueueFamilyIndices& families,
                                      u32                       width,
    ...
    bool VkSwapchainWrapper::create_internal(VkPhysicalDevice          physical,
                                             VkDevice                  device,
                                             VkSurfaceKHR              surface,
                                             const QueueFamilyIndices& families,
                                             u32                       width,
                                             u32                       height)
    {
        ...
        for (u32 i = 0; i < actual_count; ++i)
        {
            VK_CHECK_RETURN(vkCreateSemaphore(device, &sem_info, nullptr, &render_finished_semaphores_[i]), false);
        }

        ROVER_LOG_INFO("Vulkan swapchain created ({}x{}, {} images)", extent_.width, extent_.height, actual_count);
        return true;
    }
    ...
    bool VkSwapchainWrapper::acquire_next_image(VkDevice device, VkSemaphore signal_sem, u32* image_index_out)
    {
        const VkResult result =
            vkAcquireNextImageKHR(device, swapchain_, UINT64_MAX, signal_sem, VK_NULL_HANDLE, image_index_out);
