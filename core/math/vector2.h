...
struct Vector2 {
    ...
    [[nodiscard]] constexpr Vector2 operator/(f32 s) const noexcept { return {v / s}; }
    constexpr Vector2& operator+=(const Vector2& rhs) noexcept { v += rhs.v; return *this; }
    constexpr Vector2& operator-=(const Vector2& rhs) noexcept { v -= rhs.v; return *this; }
    constexpr Vector2& operator*=(f32 s) noexcept { v *= s; return *this; }
    constexpr Vector2& operator/=(f32 s) noexcept { v /= s; return *this; }
    ...
    [[nodiscard]] constexpr f32 cross(const Vector2& other) const noexcept { return v.x * other.v.y - v.y * other.v.x; }
...
[[nodiscard]] inline constexpr Vector2 operator*(f32 s, const Vector2& v) noexcept { return v * s; }
