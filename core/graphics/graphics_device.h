...
namespace rover
{
    ...
    public:
        ...
        virtual void         update_buffer(BufferHandle handle, const void* data, usize size, usize offset = 0) = 0;
        ...
        virtual BindGroupLayoutHandle create_bind_group_layout(const BindGroupLayoutDesc& desc) = 0;
        virtual void                  destroy_bind_group_layout(BindGroupLayoutHandle handle)   = 0;

        virtual BindGroupHandle create_bind_group(const BindGroupDesc& desc) = 0;
        virtual void            destroy_bind_group(BindGroupHandle handle)   = 0;

        virtual PipelineLayoutHandle create_pipeline_layout(const PipelineLayoutDesc& desc) = 0;
        ...
        virtual void cmd_begin_render_pass(CommandListHandle cmd,
        ...
        // Binds a bind group to `set_index` of the currently-bound pipeline's
        // layout. Driver implementations must validate the layout compatibility.
        virtual void cmd_bind_group(CommandListHandle cmd, u32 set_index, BindGroupHandle group) = 0;

        // Pushes constants into the layout's push-constant range. `stage` controls
        // which shader stages observe the new data.
        virtual void cmd_push_constants(CommandListHandle    cmd,
        ...
        virtual void cmd_bind_vertex_buffer(CommandListHandle cmd,
                                            BufferHandle      buffer,
                                            u32               binding = 0,
                                            usize             offset  = 0) = 0;
        virtual void cmd_bind_index_buffer(CommandListHandle cmd,
                                           BufferHandle      buffer,
                                           IndexType         type,
                                           usize             offset = 0)  = 0;

        virtual void cmd_draw(CommandListHandle cmd,
                              u32               vertex_count,
                              u32               instance_count = 1,
                              u32               first_vertex   = 0,
                              u32               first_instance = 0)         = 0;
        virtual void cmd_draw_indexed(CommandListHandle cmd,
                                      u32               index_count,
                                      u32               instance_count = 1,
                                      u32               first_index    = 0,
                                      i32               vertex_offset  = 0,
